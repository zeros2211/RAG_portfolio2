...
이 시스템은 포트폴리오 시연 목적의 로컬 서빙 환경을 전제로 설계되었습니다.

- **PDF 용량 개당 50MB 이하**: 리뷰어가 올릴 기술 문서·논문은 대부분 수 MB 수준이므로, Nginx `client_max_body_size 50m`으로 상한을 두고 업로드는 1MB 단위 스트리밍 저장으로 메모리를 절약했습니다. 100MB 이상을 지원하려면 스트리밍 파이프라인 전환과 임베딩 배치 처리가 필요합니다.
- **동시 업로드 소량 (합산 50MB 이내)**: 1~3개 문서 업로드가 자연스러운 사용 패턴이므로, BackgroundTasks 순차 처리와 in-memory 문서 레지스트리로 별도 인프라 없이 단순하게 구현했습니다. 대량 동시 업로드가 필요하면 Celery/RQ 작업 큐와 영속 레지스트리로 전환해야 합니다.
- **로컬 네트워크 (10Mbps+)**: 처리 파이프라인은 Docker 컨테이너 간 통신이라 외부 네트워크와 무관하고, 업로드만 사용자 네트워크에 의존합니다. 원격 서버 배포 시에는 resumable upload와 SSE 연결 재시도 로직이 필요합니다.
